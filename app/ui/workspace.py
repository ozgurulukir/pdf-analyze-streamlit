        ...
        for ws in workspaces:
            ...
            with col2:
                ...
                with st.expander("📝 İsmi Değiştir", expanded=False):
                    new_name = st.text_input("Yeni İsim", value=ws.name, key=f"rename_input_{ws.id}", label_visibility="collapsed")
...
def render_file_card(
    ...
    with stylable_container(
        ...
        with col_text:
            ...
            st.markdown(f"<small style='color: {color}; font-weight: 600;'>{icon} {file.status.upper()}</small> | <small style='color: #94a3b8;'>{file.size // 1024} KB • {file.chunk_count} chunk</small>", unsafe_allow_html=True)
        ...
        with btn_col1:
            if st.button("👁️ İzle", key=f"prev_{file.id}", use_container_width=True):
                on_preview(file)
        with btn_col2:
            if st.button("🔄 Sync", key=f"rev_{file.id}", use_container_width=True, disabled=file.status != "processed"):
...
def render_document_stats(files: List[FileMetadata], workspace_id: str, workspace_name: str):
    ...
    if chunk_count == 0 and processed > 0:
        st.warning("⚠️ Belgeler işlenmiş görünüyor ama vektör veritabanı boş. Lütfen 'Belgeler' sayfasından tekrar işlemeyi deneyin.")
