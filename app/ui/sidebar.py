...
def render_llm_settings() -> Dict[str, Any]:
    ...
    index = (
        endpoint_type_options.index(current_type)
        if current_type in endpoint_type_options
        else 0
    )
    ...
    if "llm_base_url_input" not in st.session_state:
        st.session_state.llm_base_url_input = st.session_state.get(
            SessionKeys.LLM_BASE_URL.value, "https://ollama.com/v1"
        )
    ...
    if current_type != endpoint_type:
        ...
        if endpoint_type == "Ollama Cloud":
            ...
            st.session_state.llm_base_url_input = st.session_state.get(
                SessionKeys.LLM_BASE_URL.value, "https://api.openai.com/v1"
            )
    ...
    api_key = st.text_input(
        ...
        help="Kimlik doğrulama anahtarı.",
        on_change=save_settings_callback,
    )
    ...
    use_custom_model = st.checkbox(
        "Özel model adı kullan",
        value=is_cloud,
        key="use_custom_llm_model",
        on_change=save_settings_callback,
    )

    if use_custom_model:
        current_model = st.session_state.get(
            SessionKeys.LLM_MODEL.value, "deepseek-v3.1:671b-cloud"
        )
        ...
        with col1:
            ...
            idx = (
                model_values.index(current_model)
    ...
    temperature = st.slider(
        ...
        0.0,
        1.0,
        float(st.session_state.get(SessionKeys.LLM_TEMPERATURE.value, 0.3)),
        0.1,
        key="llm_temp_slider",
        on_change=save_settings_callback,
    )
    ...
    if st.button("🔌 Bağlantıyı Test Et", use_container_width=True):
        ...
        try:
            with st.status("Bağlantı test ediliyor...", expanded=True) as status:
                ...
                status.update(
                    label="✅ Bağlantı Başarılı!", state="complete", expanded=False
                )
...
def render_embedding_settings() -> Dict[str, Any]:
    ...
    embed_type = st.radio(
        ...
        index=(
            0
            if not st.session_state.get(SessionKeys.USE_HUGGINGFACE.value, False)
            else 1
        ),
    ...
    if use_huggingface:
        use_custom_hf = st.checkbox(
            "Özel HF modeli kullan",
            value=False,
            key="use_custom_hf_model",
            on_change=save_settings_callback,
        )
        if use_custom_hf:
            ...
            curr_hf = st.session_state.get(
                SessionKeys.HF_EMBED_MODEL.value, HF_EMBED_OPTIONS[0]["value"]
            )
        ...
        use_custom_ollama = st.checkbox(
            "Özel Ollama embedding kullan",
            value=False,
            key="use_custom_embed_model",
            on_change=save_settings_callback,
        )
...
def render_data_settings() -> Dict[str, Any]:
    ...
    data_dir = st.text_input(
        "Veri Klasörü",
        value=st.session_state.get(SessionKeys.DATA_DIR.value, config.DATA_DIR),
        key="data_dir_input",
        on_change=save_settings_callback,
    )
    ...
    with col1:
        chunk_size = st.number_input(
            ...
            100,
            2000,
            int(st.session_state.get(SessionKeys.CHUNK_SIZE.value, 1000)),
            100,
    ...
    with col2:
        chunk_overlap = st.number_input(
            ...
            0,
            500,
            int(st.session_state.get(SessionKeys.CHUNK_OVERLAP.value, 200)),
            50,
