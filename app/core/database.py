...
class DatabaseManager:
    ...
    def delete_workspace(self, workspace_id: str) -> None:
        ...
        try:
            with self._get_connection() as conn:
                ...
                cursor.execute(
                    "DELETE FROM files WHERE workspace_id = ?", (workspace_id,)
                )
            ...
            with self._get_connection() as conn:
                ...
                """,
                    (
                        ...
                        (
                            file_meta.processed_at.isoformat()
                            if file_meta.processed_at
                            else None
                        ),
                        file_meta.error_message,
                        json.dumps(file_meta.tags),
                    ),
                )
    ...
    def get_files(self, workspace_id: str) -> List[FileMetadata]:
        ...
        try:
            ...
            raise DatabaseError(
                f"Failed to fetch files for workspace {workspace_id}: {e}"
            )
    ...
    def delete_file(self, file_id: str) -> None:
        ...
        try:
            with self._get_connection() as conn:
                ...
                cursor.execute(
                    "SELECT workspace_id FROM files WHERE id = ?", (file_id,)
                )
                row = cursor.fetchone()
    ...
    def save_preferences(self, prefs: UserPreferences) -> None:
        ...
        try:
            with self._get_connection() as conn:
                ...
                """,
                    (
                        json.dumps(prefs.weights),
                        json.dumps(prefs.config),
