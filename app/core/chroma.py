...
class ChromaManager:
    ...
    def query(
        ...
        try:
            ...
            logger.info(
                f"[Chroma] Available collections: {[c.name for c in all_collections]}"
            )
        except Exception as e:
        ...
        if collection is None:
            ...
            raise ChromaCollectionError(f"Collection '{collection_name}' not found for query.")

        logger.info(
            f"[Chroma] Collection found: {collection.name}, count: {collection.count()}"
        )
        ...
        try:
            ...
            # Safeguard against missing/empty results
        ...
        try:
            if self.use_huggingface:
                ...
                return HuggingFaceEmbeddings(
                    model_name=self.hf_model, model_kwargs={"device": "cpu"}
                )
            else:
