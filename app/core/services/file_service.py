...
class FileService:
    ...
    def upload_files(
        ...
        for uploaded_file in uploaded_files:
            try:
                ...
                existing_files = self.db.files.get_by_workspace(workspace.id)
        ...
        if files_to_process:
            ...
            workspace.file_count = self.db.files.count_by_workspace(workspace.id)
            workspace.last_modified = datetime.now()
            self.db.workspaces.update(workspace)
            successes.append(
    ...
    def delete_file(self, file_id: str, workspace_id: str) -> None:
        ...
        try:
            ...
            if workspace:
                self.db.files.delete(file_id)
                workspace.file_count = self.db.files.count_by_workspace(workspace_id)
                workspace.last_modified = datetime.now()
                self.db.workspaces.update(workspace)
